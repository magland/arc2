...
from ..dj_init import SPYGLASS_INSTANCE


def prepare_tables(tmpdir: str):
    if not os.path.exists(f'{tmpdir}/output/tables'):
        os.makedirs(f'{tmpdir}/output/tables')
    with open(f'{tmpdir}/table_names.json', 'r') as f:
        table_names = json.load(f)
    for table_name in table_names:
        fname = f'{tmpdir}/output/tables/{table_name}.json'
...
def prepare_sessions(tmpdir: str):
    if not os.path.exists(f'{tmpdir}/output/sessions'):
        os.makedirs(f'{tmpdir}/output/sessions')
    nwb_file_names = []
    with open(f'{tmpdir}/output/tables/`common_session`.`_session`.json', 'r') as f:
        x = json.load(f)
    for row in x['rows']:
    ...
    table_names = []
    for fname in os.listdir(f'{tmpdir}/output/tables'):
    ...
    all_tables = {}
    for table_name in table_names:
        with open(f'{tmpdir}/output/tables/{table_name}.json', 'r') as f:
            x = json.load(f)
    ...
    for nwb_file_name in nwb_file_names:
        ...
        with open(f'{tmpdir}/output/sessions/{nwb_file_name}.json', 'w') as f:
            json.dump(session, f, indent=4)


def upload_session_files(tmpdir: str):
    for fname in os.listdir(f'{tmpdir}/output/sessions'):
        if not fname.endswith('.json'):
            continue
        _upload_file(f'{tmpdir}/output/sessions/{fname}', f'spyndle/{SPYGLASS_INSTANCE}/sessions/{fname}')
